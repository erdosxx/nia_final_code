...
for id in id_list:
    ...
    for ac_num in range(1,5):
        for cam_num in range(1,5):
            frame_list = glob.glob('GMS/'+ id +'/' + id + '_GMS_' + str(ac_num) + '_' + str(cam_num) + '*.json')
            ...
            for i, frame in enumerate(frame_list):
                ...
                if i == 0:
                    if args.label == 'action':
                        ...
                        video_info['label_index'] = int(data['metaData']['beh_eval_code'])
                    else:
                        video_info['ID'] = str(data['metaData']['ID'])
                ...
                body_points = [[j for j in k.values()] for k in data['labelingInfo'][0]['pose']['location'].values()]
            ...
            with open('./processed/'+ args.age + '/' + video_info['ID']+'GMS_'+str(ac_num) + '_' + str(cam_num) + '.json', 'wt' ) as f:
